...
float3 perturb_normal(float3 N, float3 V, float2 texcoord) {
	...
	float3 map = 2.0f * heightmap.Sample(hmsampler, texcoord).yzw - 1.0f;
...
float3 estimateNormal(float2 texcoord) {
	...
	float2 b = texcoord + float2(0.0f, -1 / (float)height);
	float2 c = texcoord + float2(1 / (float)width, 1 / (float)height);
	float2 d = texcoord + float2(1 / (float)width, 0.0f);
	float2 e = texcoord + float2(1 / (float)width, 1 / (float)height);
	float2 f = texcoord + float2(0.0f, 1 / (float)height);
	float2 g = texcoord + float2(-1 / (float)width, 1 / (float)height);
	float2 h = texcoord + float2(-1 / (float)width, 0.0f);
	float2 i = texcoord + float2(-1 / (float)width, -1 / (float)height);
...
//    return heightmap.Sample(hmsampler, input.tex);
	...
	float3 N = estimateNormal(input.tex);
	//float3 viewvector = eye.xyz - input.worldpos;
//	N = perturb_normal(N, viewvector, input.tex / 16.0f);
	float4 norm = float4(N, 1.0f);

	float diffuse = saturate(dot(norm, -light));
    float ambient = 0.2f;
